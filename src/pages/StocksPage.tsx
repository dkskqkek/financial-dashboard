...
export function StocksPage() {
  ...
  const updateStockTotals = React.useCallback(async (stocksToCalculate: Stock[]) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className={`text-xs ${getColorByValue(totalReturn)}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getColorByValue(totalReturn)}`}>
              {formatPercent(totalReturn)}
            </div>
            <p className="text-xs text-muted-foreground">
              연환산 기준
              ...
              {(sortedStocksKrw.length > 0 ? sortedStocksKrw.map(({stock}) => stock) : filteredStocks).map((stock) => {
                ...
                return (
                  <TableRow key={stock.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <Badge variant={stock.exchange === 'KRX' ? 'default' : 'secondary'}>
                        {stock.exchange}
                      </Badge>
                    </TableCell>
      ...
      <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="quantity" className="text-sm font-medium">보유 수량</label>
              <Input
                id="quantity"
                type="number"
            ...
            <div>
              <label htmlFor="averagePrice" className="text-sm font-medium">평균 매입가</label>
              <Input
                id="averagePrice"
                type="number"
            ...
            <div>
              <label htmlFor="sector" className="text-sm font-medium">섹터</label>
              <select
                id="sector"
